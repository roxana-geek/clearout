...
>
{copied ? “v Скопировано” : “Копировать”}
</button>
);
}
...
>
<span style={{ fontSize: “14px”, fontWeight: 700 }}>{emoji}</span>
{label}
</a>
);
}
...
? “https://t.me/izrail_avito”
: platform.id === “facebook”
? “https://www.facebook.com/marketplace/create/item/”
: “https://www.yad2.co.il/post”;

const shareLabel =
platform.id === “yad2”
? “Открыть Yad2”
: platform.id === “telegram”
? “Открыть барахолку Израиль”
: “Открыть Facebook”;
...
<span style={{ fontFamily: “‘DM Mono’, monospace”, fontWeight: 700, fontSize: “12px”, color: “#1a1a2e” }}>
{platform.emoji}
</span>
<span
style={{
...
```
  {isLoading ? (
    ...
    <div style={{ color: "#bbb", fontSize: "13px", fontFamily: "'DM Sans', sans-serif", padding: "6px 0" }}>
      Объявление появится здесь
    </div>
  )}
</div>
```
...
try {
  ...
  let item;
  try {
    item = JSON.parse(raw.replace(/```json|```/g, "").trim());
  } catch {
    item = { name: "Товар", condition: 8, keyFeatures: [], suggestedPriceILS: parseInt(price) || 500 };
  }
  ...
  const ctx = `Item: ${item.name} | Category: ${item.category} | Condition: ${item.condition}/10 | Features: ${(item.keyFeatures || []).join(", ")} | Price: ${p} ILS | Min: ${mp} ILS | Notes: ${notes || "Moving to new home, selling quickly"}`;

  setLoading({ yad2: true, telegram: true, facebook: true });

  await Promise.all([
    callClaude(
      "Write a Yad2 Israeli classified ad in Hebrew. Max 10 lines. Format: title with icon, blank line, bullet features with dash, blank line, price line mentioning moving sale. Be concise.",
      `Write listing for: ${ctx}`,
      null
    ).then((t) => {
      setListings((prev) => ({ ...prev, yad2: t }));
      setLoading((prev) => ({ ...prev, yad2: false }));
    }),

    callClaude(
      "Write a Telegram classified ad in Russian for Israeli Russian-speaking groups. Max 10 lines. Format: CAPS title, blank line, dash bullets, blank line, price. Warm and friendly.",
      `Write listing for: ${ctx}`,
      null
    ).then((t) => {
      setListings((prev) => ({ ...prev, telegram: t }));
      setLoading((prev) => ({ ...prev, telegram: false }));
    }),

    callClaude(
      "Write a Facebook Marketplace listing in English. Max 10 lines. Format: catchy title, blank line, dash bullets, blank line, price + Pickup only. Clear and friendly.",
      `Write listing for: ${ctx}`,
      null
    ).then((t) => {
      setListings((prev) => ({ ...prev, facebook: t }));
...
<style>{`@import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@300;400;500;600&family=DM+Mono:wght@400;500;600&family=Playfair+Display:wght@700;800&display=swap'); @keyframes spin { to { transform: rotate(360deg); } } @keyframes fadeUp { from { opacity:0; transform:translateY(14px); } to { opacity:1; transform:translateY(0); } } * { box-sizing: border-box; } input:focus, textarea:focus { border-color: #f0c040 !important; }`}</style>
  ...
  <div style={{ maxWidth: "560px", margin: "0 auto", padding: "24px 16px 60px" }}>
    <div
      ...
      style={{
        ...
        borderRadius: "20px",
        background: image ? "#fff" : dragOver ? "#fffbeb" : "#fff",
        cursor: "pointer",
        transition: "all 0.2s",
        overflow: "hidden",
        marginBottom: "16px",
        minHeight: image ? "auto" : "140px",
        display: "flex",
        alignItems: "center",
        justifyContent: "center",
      }}
    >
      ...
      {image ? (
        ...
        <div style={{ textAlign: "center", padding: "28px 20px" }}>
          ...
          <div style={{ fontSize: "11px", color: "#999", marginTop: "5px" }}>или перетащи фото сюда</div>
        </div>
      )}
    </div>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", marginBottom: "10px" }}>
      {[["Цена (₪)", price, setPrice, "950"], ["Мин. цена (₪)", minPrice, setMinPrice, "870"]].map(([label, val, set, ph]) => (
        <div key={label}>
          <label style={{ display: "block", fontFamily: "'DM Mono', monospace", fontSize: "10px", letterSpacing: "0.1em", textTransform: "uppercase", color: "#888", marginBottom: "5px" }}>
            {label}
          </label>
    ...
    <div style={{ marginBottom: "18px" }}>
      ...
      <textarea
        ...
        placeholder="Куплено за 920₪, использовалась мало, переезд..."
        rows={2}
    ...
    <button
      onClick={generate}
      disabled={!imageBase64 || isWorking}
      style={{
        width: "100%",
        padding: "15px",
    ...
    {itemSummary && (
      <div style={{ background: "#1a1a2e", borderRadius: "14px", padding: "14px 18px", marginBottom: "18px", animation: "fadeUp 0.4s ease" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "8px" }}>
          <div>
            <div style={{ fontFamily: "'Playfair Display', serif", color: "#f0c040", fontSize: "16px", fontWeight: 700 }}>
              {itemSummary.name}
            </div>
          ...
          <div style={{ background: "#f0c040", color: "#1a1a2e", borderRadius: "8px", padding: "5px 12px", fontFamily: "'DM Mono', monospace", fontWeight: 700, fontSize: "15px" }}>
            {price || itemSummary.suggestedPriceILS} ILS
          </div>
        </div>
      </div>
    )}
